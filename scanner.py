...
def normalize_title(title: str) -> str:
    """Strip clean/edited suffixes and normalize for comparison."""
    return CLEAN_SUFFIXES.sub('', title).strip().lower()


def normalize_artist(name: str) -> str:
    """Lowercase and strip punctuation for lenient artist comparison."""
...
def _search_with_retry(yt: YTMusic, query: str) -> list[dict] | None:
    """Search YouTube Music with one retry on failure/empty results."""
    time.sleep(SEARCH_DELAY)

    for attempt in range(2):
        try:
            results = yt.search(query, filter="songs", limit=SEARCH_LIMIT)
...
def _filter_and_pick_best(
    ...
    for result in results:
        ...
        if normalize_title(result.get("title", "")) != title_normalized:
            continue

        result_artists = result.get("artists") or []
        result_artist_names = " ".join(a.get("name", "") for a in result_artists)
        if artist_normalized not in normalize_artist(result_artist_names):
            continue
...
def _find_match(
    ...
    logger.debug("Searching for match: %s (explicit_only=%s)", query, require_explicit)

    results = _search_with_retry(yt, query)
    if not results:
        return None
...
def _find_match_with_title_fallback(
    ...
    match = _find_match(yt, track, require_explicit)
    if match:
        return match
...
def find_available_match(yt: YTMusic, track: TrackInfo) -> TrackInfo | None:
    """Search for any available version of an unavailable track (explicit preferred)."""
    return _find_match_with_title_fallback(yt, track, require_explicit=False)
...
def scan_playlist(
    ...
    for i, raw_track in enumerate(tracks):
        ...
        if not track.is_available:
            ...
            match = find_available_match(yt, track)
            if match:
